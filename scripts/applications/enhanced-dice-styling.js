export class EnhancedDiceStyling {
  ...
  static _styleDiceTooltips(html) {
    const tooltipParts = html.find('.dice-tooltip .wrapper .roll-dice .roll-die');
  ...
  static _styleChatMessageBackground(html, message) {
    ...
    const chatMessage = html.hasClass('chat-message') ? html : html.closest('.chat-message');
  ...
  static _addClickableRerollHandlers(html, message) {
    ...
    if (diceTooltip.length > 0 && !diceTooltip.find('.reroll-instruction').length) {
  ...
  static async _handleDiceReroll(dieElement, dieType, message) {
    ...
    try {
      ...
      const rerolledIcon = $('<span class="dice-rerolled"><i class="fas fa-redo"></i></span>');
      ...
      ui.notifications.info(`${dieType.charAt(0).toUpperCase() + dieType.slice(1)} die rerolled!`);
